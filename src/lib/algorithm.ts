...
function startAlgo(
	...
	if (vKeyMax.keys.length === 1) {
		// Vincitore singolo
		return assignObject(map, vKeyMax.keys[0], oneObj, users);
		...
		do {
			...
			const subResult = startAlgo(strObj_X, oneObj, users);
		...
		if (keysCopy.length === 1) {
			...
			return assignObject(map, keysCopy[0], oneObj, users);
		} else {
			// Fallback: chooseKey
			return assignObject(map, chooseKey(keysCopy, users), oneObj, users);
		}
	}
...
export function runSwap(
	...
	// Ordina oggetti
	const sortedObjects = sortObjects(objectSwap, params.order);

	// Esegui algoritmo
	for (const obj of sortedObjects) {
		const result = startAlgo(obj.key, params.oneObj, users);
		if (result) {
			results.push(result);
		}
	}
